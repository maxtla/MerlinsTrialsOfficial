...
public:
	...
	//PUBLIC METHODS
	//set functions
	void setIsFloating(bool value) { isFloating = value; }
	...
	void setTerrainHeight(float height) { this->terrainHeight = height; }
	void updateFrustum(Matrix rotation);
	...
	Vector3 getCamForward() const { return XMVectorSubtract(m_target, m_camPos); /*m_camForward;*/ }
	...
	DirectX::BoundingFrustum* getFrustum() { return frustrum; }
...
private:
	//PRIVATE MEMBERS
	HWND mHwnd;
	POINT mouseLast, mouseNew;
	RECT windowRect; //our boundaries
	//Matrices
	Matrix m_view;
	Matrix m_projection;
	Matrix m_orthoProj;
	...
	DirectX::BoundingFrustum setLHFrustrum();
