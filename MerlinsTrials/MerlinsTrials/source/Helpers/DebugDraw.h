...
public:
	BoundingBoxShader() {}
	~BoundingBoxShader() { delete pDebugShader; debugBatch.reset(); }
	BasicShader * pDebugShader = nullptr;
	std::unique_ptr<PrimitiveBatch<VertexPositionColor>> debugBatch;
};

static BoundingBoxShader BBSHADER;

namespace DebugDrawShape
{
	inline void XM_CALLCONV DrawCube(BasicShader * pDebugShader,
		PrimitiveBatch<VertexPositionColor> *debugBatch,
		Matrix view, Matrix proj,
		CXMMATRIX matWorld,
		FXMVECTOR color)
	{
		pDebugShader->m_effect->SetMatrices(Matrix::Identity, view, proj);
		pDebugShader->setBlendState(OPAQUE_STATE);
		pDebugShader->setDepthStencilState(DEPTH_NONE_STATE);
		pDebugShader->setRasterizerState(CULL_NONE_STATE);
		pDebugShader->Apply();
		...
		debugBatch->Begin();
		debugBatch->DrawIndexed(D3D11_PRIMITIVE_TOPOLOGY_LINELIST, s_indices, _countof(s_indices), verts, 8);
		debugBatch->End();
	}

	inline void XM_CALLCONV DrawRing(BasicShader * pDebugShader,
		PrimitiveBatch<VertexPositionColor> *debugBatch,
		Matrix view, Matrix proj,
		FXMVECTOR origin,
		FXMVECTOR majorAxis,
		...
		GXMVECTOR color)
	{
		...
		debugBatch->Begin();
		debugBatch->Draw(D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP, verts, c_ringSegments + 1);
		debugBatch->End();
	}
}
...
namespace DebugDraw
{
	inline void XM_CALLCONV Draw(BasicShader * pDebugShader,
		PrimitiveBatch<VertexPositionColor > *debugBatch,
		Matrix view, Matrix proj,
		...
		DirectX::FXMVECTOR color = DirectX::Colors::White)
	{
		...
		DebugDrawShape::DrawCube(pDebugShader, debugBatch, view, proj, matWorld, color);
		...
		FXMVECTOR color)
	{
		...
		DebugDrawShape::DrawRing(pDebugShader, debugBatch, view, proj, origin, xaxis, zaxis, color);
		DebugDrawShape::DrawRing(pDebugShader, debugBatch, view, proj, origin, xaxis, yaxis, color);
		DebugDrawShape::DrawRing(pDebugShader, debugBatch, view, proj, origin, yaxis, zaxis, color);
	}
