...
void Mesh::DrawDebugMode(BasicShader * pDebugShader,
	...
	Matrix view, Matrix proj, Vector4 color)
{
	...
	{
		if (hasOBB)
			DebugDraw::Draw(pDebugShader, debugBatch, view, proj, m_OBB, color);
		else if (hasSphere)
			DebugDraw::Draw(pDebugShader, debugBatch, view, proj, m_Sphere, color);
		else if (hasAABB)
			DebugDraw::Draw(pDebugShader, debugBatch, view, proj, m_AABB, color);
	}
}
...
void Mesh::createBoundingShapeFromType(int type, float scaling)
{
...
bool Mesh::collisionTest(BoundingOrientedBox * avatar)
{
	if (this->hasOBB)
	{
		 return avatar->Intersects(m_OBB);
	}
	if (this->hasAABB)
	{
		return avatar->Intersects(m_AABB);
	}
	if (this->hasSphere)
	{
		return avatar->Intersects(m_Sphere);
	}
	return false;
}

bool Mesh::collisionTestFrustrum(BoundingFrustum * frustrum)
{
	if (this->hasOBB)
	{
		return frustrum->Intersects(m_OBB);
	}
	if (this->hasAABB)
	{
		return frustrum->Intersects(m_AABB);
	}
	if (this->hasSphere)
	{
		return frustrum->Intersects(m_Sphere);
	}
	if (this->hasNoBoundingShape || this->isTerrain)
	{
		return true;
	}
	return false;
