...
{
	CollisionBox worldBox = *this;
	XMVECTOR Center = XMLoadFloat3(&worldBox.box.Center);
	XMVECTOR Extents = XMLoadFloat3(&worldBox.box.Extents);
	XMVECTOR Orientation = XMLoadFloat4(&worldBox.box.Orientation);
	XMVector3TransformCoord(Center, world);
	XMVector3Transform(Extents, world);
	XMVector4Transform(Orientation, world);
	XMStoreFloat3(&worldBox.box.Center, Center);
	XMStoreFloat3(&worldBox.box.Extents, Extents);
	DirectX::XMStoreFloat4(&worldBox.box.Orientation, Orientation);

	worldBox.generateFaces();
	return worldBox;
}