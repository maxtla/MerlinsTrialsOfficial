...
void Direct3DEngine::resetEngine()
{
	...
	this->pDevCon->ClearRenderTargetView(this->m_pRTV, color);
	this->pDevCon->ClearDepthStencilView(this->m_pDSV, D3D11_CLEAR_DEPTH, 1.0f, 0);

	//prepare render target view and dsv state
	this->pDevCon->OMSetRenderTargets(1, &this->m_pRTV, this->m_pDSV);
	this->pDevCon->OMSetDepthStencilState(this->m_pDSS, 1);

	//prepare rasterizer
	this->pDevCon->RSSetState(this->m_pRS);
	this->pDevCon->RSSetViewports(1, &this->m_viewPort);
}

void Direct3DEngine::Release()
{
	if (pDev)
		pDev.Reset();
	if (pDevCon)
...
bool Direct3DEngine::initDeviceAndSwapChain(HWND hwnd)
{
	...
	if (GetWindowRect(hwnd, &rect))
	{
		...
		hr = m_pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&this->m_pBackBuffer);
		if (FAILED(hr))
			return false;
	}
	else
...
bool Direct3DEngine::initRenderTargetView()
{
	...
	hr = pDev->CreateRenderTargetView(this->m_pBackBuffer, NULL, &this->m_pRTV);
	if (FAILED(hr))
		return false;
...
bool Direct3DEngine::initDepthStencilView(HWND hwnd)
{
	...
	if (GetWindowRect(hwnd, &rect))
	{
		...
		hr = pDev->CreateDepthStencilView(this->m_pDB, &dsvDesc, &this->m_pDSV);
		if (FAILED(hr))
			return false;
	}
	else
