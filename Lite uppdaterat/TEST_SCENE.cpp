...
void TEST_SCENE::Init(Direct3DEngine * p3DEngine, Direct2DEngine * p2DEngine,InputHandler * pInputHandler)
{
	...
	pPedestalManager = new PedestalManager();
	levelOne = new Level();
	...
	if (!pPedestalManager->initialize(pMeshManager->getPedestals(), p3DEngine->pDevCon.Get(), p3DEngine->pDev.Get()))
		exit(-1);
...
void TEST_SCENE::Draw()
{
	...
	pPedestalManager->Draw(pCamera->getView(), pCamera->getProjection());
